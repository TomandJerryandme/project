...
def __import__(name, globals=None, locals=None, fromlist=(), level=0):
    """
        name: 被导入的包名或者模块名或者方法名等
        globals: 用于推断导入语句的位置，用于处理相对导入
        locals: 已经废弃，python3里面没有用到
        fromlist: eg: from module import <fromlist>
            可以如下使用： __import__(module_name, fromlist=[module_name中的变量、方法等])
        level: 相对导入使用参数 
            eg: from ..pkg import mod  其中，使用了__import__()
    """
    if level == 0:
